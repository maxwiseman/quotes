...
export function ResultCard(resultData: { title: string | null, description: string | null, number: number | null, id: number, quotes: { text: string, startTime: number, endTime: number, id: number, index: number }[] }): React.ReactElement {
  return (
    ...
    <Card>
      ...
      <CardContent className="flex gap-4 flex-col p-4">
        {resultData.quotes.map((quote) => (
          <Link key={quote.id} href={`/episode/${resultData.id.toString()}?qIdx=${quote.index.toString()}`}>
            <Quote withPlayButton={false} key={quote.id} text={quote.text} startTime={quote.startTime} endTime={quote.endTime} id={quote.id} />
          </Link>
        ))}
      </CardContent>
    </Card>
